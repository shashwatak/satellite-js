...
import {
  BulkPropagator, EcfPositionCalculator, EciBaseCalculator, GmstCalculator, LookAnglesCalculator,
} from '../src/wasm/index.js';
import {
  ecfToLookAngles, eciToEcf, gstime, json2satrec, propagate, SatRecError,
} from '../src/index.js';
...
describe('LookAngles', () => {
  ...
  bench('LookAngles Pure JS propagate loop', () => {
    let local = 0;
    for (const date of dates) {
      ...
      for (const satrec of satrecs) {
        ...
        if (pv) {
          ...
          const lookAngles = ecfToLookAngles({ longitude: 0, latitude: 0, height: 0 }, positionEcf);
