...
extern "C" {
  ...
  // }
	...
	void EMSCRIPTEN_KEEPALIVE calculate_eci_base(elsetrec* __restrict satellites, int satellites_count, double* __restrict dates_unix_ms, int dates_count, double* __restrict eci_positions, double* __restrict eci_velocities, int* __restrict sgp4_errors) {
  ...
  double unix_ms_to_jday(double unix_ms) {
    ...
    for (int i = 0; i < satellites_count; i++) {
      for (int j = 0; j < dates_count; j++) {
        ...
        double topS = ((sin(latitude) * cos(longitude) * rx)
            + (sin(latitude) * sin(longitude) * ry))
          - (cos(latitude) * rz);

        double topE = (-sin(longitude) * rx)
          + (cos(longitude) * ry);

        double topZ = (cos(latitude) * cos(longitude) * rx)
