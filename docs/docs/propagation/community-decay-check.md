...
This check **does not exist in the official SGP4 algorithm**. While it's very likely that this is the behavior you actually want, it will NOT be strictly compliant to the reference SGP4 output, but the non-compliance is limited to the cases of satellites decayed long ago.
:::
...
if (satrec.error === SatRecError.Decayed) {
...
Call it after a successful `sgp4()` (or `propagate()` without `communityDecayCheckEnabled` flag) to test whether the satellite actually has decayed:
...
if (state && checkForDecay(satrec)) {
  // SGP4 reported successful propagation, but the satellite has actually decayed.
}
```
...
The check was authored by [Theodore Kruczek](https://github.com/thkruz) for his library [OOTK](https://github.com/thkruz/ootk). OOTK is licensed under AGPL, which means code from there normally can't be used in MIT-licensed Satellite.js, but for this check his [explicit permission](https://github.com/thkruz/ootk/issues/31#issuecomment-4893495872) was obtained.
