...
The Satellite.js library has originally started as a port of the [Python sgp4 library](https://pypi.org/project/sgp4/), which in turn has its roots in [the C++ code by David Vallado](https://github.com/CelesTrak/fundamentals-of-astrodynamics). The library is pure calculations - and it's exactly one of the main use cases for WASM.

## Prior art

There were at least 2 previous attempts to make SGP4 on WASM, one of which was never open-sourced, and the second, Rust-based, was apparently abandoned a few years ago due to the worse performance than expected.
