...
import { allocateAndWriteNativeStructArrayFromSatrecArray } from './native-structs-from-js.js';
 ...
 *     new LookAnglesCalculator() // depends on EcfPosition; requires observer parameter in propagator.run()
 *   ],
 ...
 * 
 * The approach above is the same as pure javascript below, but BulkPropagator is much more performant.
 * const results = [];
 ...
 * ```
 *
 * @template Calculators - Readonly array of Calculator instances that define the computations
 */
export class BulkPropagator<const Calculators extends readonly Calculator<string, number, TupleOf<string, number>, TypedArray | Record<string, TypedArray>, unknown>[]> implements Disposable {
  ...
  /**
   * Creates a BulkPropagator instance. The BulkPropagator is generic depending on the Calculator instances passed to it.
   * The return types of outputs, and argument type of `run()` depend on the passed Calculators.
   * 
   * @param options - Configuration options
   * @param options.wasmModule - The compiled WebAssembly module instance (use createWasmModule to create one and reuse it)
   * @param options.calculators - Array of calculator instances to run during propagation; they all named as *Calculator for easy discovery
   * @param options.satRecs - readonly array of SatRec objectsv
   ...
   *   datesCount: 24 // Will propagate up to 24 time points
   * });
   * ```
   * 
   * @throws {Error} If calculator dependencies cannot be resolved. Consult specific calculator type to understand the dependencies.
   */
  constructor({
    wasmModule,
    calculators,
    ...
    this.satrecsPointer = allocateAndWriteNativeStructArrayFromSatrecArray(wasmModule, satRecs);
   ...
   * Overwrites previous results since allocalted memory is reused.
   * 
   * @param args - Run arguments including dates and calculator-specific parameters
   * @param args.dates - Array of Date objects to propagate satellites for (length must match datesCount from constructor)
   * @param args[calculatorName] - Some calculators require additional parameters (example: LookAnglesCalculator requires observer position).
   * 
   * @example
   * ```typescript
   ...
   * ```
   * 
   * @throws {Error} If dates array length doesn't match the datesCount from constructor
   */
   ...
   * @returns Formatted output object with results from all calculators, or undefined if indices are out of bounds
   * 
   * @example
   * ```typescript
   ...
   * 
   * This method is automatically called when using the `using` declaration (if Symbol.dispose is supported).
   ...
   * ```
   * 
   * @throws {Error} If called multiple times (memory already freed)
   */
  dispose(): void {
    this.checkIfDisposed();
